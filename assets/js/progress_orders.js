  ...
  $(document).ready(function () {
      ...
      $.ajax({
        url: app_url+'index.php/service_orders_controller/get_all_in_progress_data',
        type: 'POST',  
      })
          .done(function(data) {
        ...
        if (output.status == 200) {  
          ...
          for (var i = 0; i < output.data.length; i++) {
            ...
            }else if( role_id == 1 && output.data[i].status == 3 ) {
               console.log(role_id, output.data[i].status)
              buttons = view_btn;


            }else if (role_id == 2 && output.data[i].status == 3 ) {
              console.log(role_id, output.data[i].status)
              buttons = view_btn;
    ...
    $("#myModal").addClass('progress_orders');
    ...
    .done(function(data) {
      ...
      if (output.status == 200) {
        ...
        $('.progress_orders #service_order_no').html(output.data.service_order_no);
        $('.progress_orders #order_date').html(output.data.order_date);
        $('.progress_orders #first_name').html(output.data.first_name);
        $('.progress_orders #last_name').html(output.data.last_name);
        $('.progress_orders #contact_no').html(output.data.contact_no);
        $('.progress_orders #machine_model').html(output.data.machine_model);
        $('.progress_orders #serial_no').html(output.data.serial_no); 
        $('.progress_orders #accessories').html(output.data.accessories);  
        $('.progress_orders #remarks').html(output.data.remarks);  
        $('.progress_orders #status').html(output.data.status); 
        ...
        $('.progress_orders #status').html(label);
